const WeatherApp = class {
    ...
    drawWeather() {
        ...
        if (this.currentWeather) {
            ...
            const windSpeed = Math.round(this.currentWeather.wind.speed*3.6);
            const description = this.currentWeather.weather[0].description;

            const weatherBlock = this.createCurrentWeatherBlock(dateTimeString, temperature, feelsLikeTemperature, pressure, visibility, humidity, windSpeed, iconName, description);
    ...
    createCurrentWeatherBlock(dateString, temperature, feelsLikeTemperature, pressure, visibility, humidity, windSpeed, iconName, description) {
        let weatherBlock = document.createElement("div");
        weatherBlock.className = "weather-current-block";
        ...
        pressureBlock.innerText = `Ciśnienie: ${pressure} hPa`;
        weatherBlock.appendChild(pressureBlock);

        let visibilityBlock = document.createElement("div");
        visibilityBlock.className = "weather-visibility";
        visibilityBlock.innerText = `Widoczność: ${visibility} m`;
        weatherBlock.appendChild(visibilityBlock);

        let humidityBlock = document.createElement("div");
        humidityBlock.className = "weather-humidity";
        humidityBlock.innerText = `Wilgotność ${humidity}%`;
        weatherBlock.appendChild(humidityBlock);

        let windSpeedBlock = document.createElement("div");
        windSpeedBlock.className = "weather-wind-speed";
        windSpeedBlock.innerText = `Wiatr: ${windSpeed} km/h`;
        weatherBlock.appendChild(windSpeedBlock);
